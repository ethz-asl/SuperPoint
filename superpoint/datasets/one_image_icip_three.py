...
class OneImageIcipThree(BaseDataset):
    default_config = {
        'labels': None,
    ...
    def _init_dataset(self, **config):
        base_path_opt = Path(DATA_PATH, 'one_image_icip_three/optical')
        image_paths_opt = sorted(list(base_path_opt.iterdir()))
        if config['truncate']:
            image_paths_opt = image_paths_opt[:config['truncate']]
        
        base_path_ir = Path(DATA_PATH, 'one_image_icip_three/thermal')
    ...
    def _get_data(self, files, split_name, **config):
        ...
        # Data augmentation (relevant for SuperPoint and MagicPoint training, not exporting detections)
